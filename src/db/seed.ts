...
const statuses = ['PUBLISHED', 'PUBLISHED', 'PUBLISHED', 'UNPUBLISHED', 'DRAFT'];

const countries = ['Germany', 'Ukraine', 'France', 'Austria', 'Switzerland', 'Poland', 'Netherlands'];
...
const initiatives = [
  { name: 'Berlin Odessa Express', description: 'Humanitarian aid delivery to Ukraine since 2022.' },
  { name: 'Ride for Water, Ride for Life', description: '2,000 km bike tour raising funds for clean water access.' },
  { name: 'Green Future Initiative', description: 'Planting forests and restoring ecosystems across Europe.' },
  { name: 'Education for All', description: 'Providing school materials and scholarships to underprivileged children.' },
  { name: 'Medical Aid Network', description: 'Delivering critical medical supplies to underserved communities.' },
  { name: 'Animal Rescue Alliance', description: 'Rescuing and rehoming animals in crisis zones.' },
  { name: 'Cultural Heritage Fund', description: 'Preserving historic sites and cultural traditions.' },
  { name: 'Youth Sports Program', description: 'Empowering youth through sport and physical activity.' },
];
...
const enterpriseNames: (string | null)[] = [
  null,
  null,
  'WE AID gGmbH',
  'Aktion Sühnezeichen',
...
function makeDescription(projectName: string): string {
...
async function seed() {
  ...
  const records = projectNames.map((name) => {
    ...
    const remainingBalance = (parseFloat(targetAmount) - parseFloat(totalDonation)).toFixed(2);
    const enterpriseName = pick(enterpriseNames);
    const publishedAt = status !== 'DRAFT' ? new Date(Date.now() - randInt(1, 365) * 86400000) : null;
    ...
    return {
      ...
      projectFeeType: pick(['PERCENTAGE', 'FIXED'] as const),
      projectStatus: status as 'DRAFT' | 'PUBLISHED' | 'ARCHIVED' | 'CLOSED' | 'UNPUBLISHED',
